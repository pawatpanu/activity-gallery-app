...
### Standalone Provider Login

Photoview can also be deployed as a standalone application that authenticates users through an external Provider-based login flow.
This integration is self-contained in Photoview itself and does not require files from any other project.

To enable it:

1. Set the Provider-related environment variables in `.env`:
   - `PHOTOVIEW_PROVIDER_AUTH_ENABLED=1`
   - `PHOTOVIEW_PROVIDER_AUTO_PROVISION=1`
   - `PHOTOVIEW_PROVIDER_HEALTH_ID_CLIENT_ID`
   - `PHOTOVIEW_PROVIDER_HEALTH_ID_CLIENT_SECRET`
   - `PHOTOVIEW_PROVIDER_HEALTH_ID_REDIRECT_URI`
   - `PHOTOVIEW_PROVIDER_CLIENT_ID`
   - `PHOTOVIEW_PROVIDER_SECRET_KEY`
2. For a brand-new installation, set `PHOTOVIEW_INITIAL_ROOT_PATH=/photos` if you want the first successful Provider login to complete initial setup automatically.
3. Make sure the callback URL points to this Photoview instance, for example:
...
- Auto-provisioning creates a local Photoview user record from the Provider identity the first time that user signs in.
- For production, prefer deploying Photoview behind HTTPS so callback URLs and cookies behave correctly.
...
The graphql playground can now be accessed at [localhost:4001/api](http://localhost:4001/api). The site can now be accessed at [localhost:1234](http://localhost:1234). Both servers will be relaunched after the code is changed.
...
The graphql playground can now be accessed at [localhost:4001/api](http://localhost:4001/api).
