...
func logUIendpointURL() {
	...
	if len(uiEndpoints) == 0 {
		log.Printf("Photoview UI public endpoint not configured. Set %s or %s if the UI is served separately.\n",
			utils.EnvUIEndpoints.GetName(), utils.EnvUIEndpoint.GetName())
	} else if len(uiEndpoints) == 1 {
		log.Printf("Photoview UI public endpoint ready at %s\n", uiEndpoints[0].String())
	} else {
		log.Printf("Photoview UI public endpoints configured via %s\n", utils.EnvUIEndpoints.GetName())
