...
type User struct {
	...
	AuthProvider   *string `gorm:"size:64;index"`
	AuthProviderID *string `gorm:"size:191;uniqueIndex"`
...
func FindUserByAuthProvider(db *gorm.DB, provider string, providerID string) (*User, error) {
	var user User
	result := db.Where("auth_provider = ? AND auth_provider_id = ?", provider, providerID).First(&user)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, errors.Wrap(result.Error, "find user by auth provider")
	}

	return &user, nil
}

func RegisterExternalUser(db *gorm.DB, username string, provider string, providerID string, admin bool) (*User, error) {
	user := User{
		Username:       username,
		Admin:          admin,
	...
	result := db.Create(&user)
	if result.Error != nil {
		return nil, errors.Wrap(result.Error, "insert external user into database")
	}
